...
void GLCD_SPIM_UDB_IntClock_SetFractionalDividerRegister(uint16 clkDivider, uint8 clkFractional)
{
    ...
    uint32 maskVal = GLCD_SPIM_UDB_IntClock_DIV_REG & 
                    (uint32)(~(GLCD_SPIM_UDB_IntClock__DIVIDER_MASK | GLCD_SPIM_UDB_IntClock__FRAC_MASK)); 
	/* combine mask and new divider val into 32-bit value */
    uint32 regVal = clkDivider | (((uint32)clkFractional << 16) & GLCD_SPIM_UDB_IntClock__FRAC_MASK) | maskVal;
#else
    /* get all but integer divider bits */
    uint32 maskVal = GLCD_SPIM_UDB_IntClock_DIV_REG & (uint32)(~(uint32)GLCD_SPIM_UDB_IntClock__DIVIDER_MASK);
    /* combine mask and new divider val into 32-bit value */
...
{
    return (uint16)GLCD_SPIM_UDB_IntClock_DIV_REG;
}
...
uint8 GLCD_SPIM_UDB_IntClock_GetFractionalDividerRegister(void)
{
    ...
    uint32 maskVal = GLCD_SPIM_UDB_IntClock_DIV_REG & GLCD_SPIM_UDB_IntClock__FRAC_MASK;
    return (maskVal >> 16);
#else
    return 0u;
#endif /* GLCD_SPIM_UDB_IntClock__FRAC_MASK */
